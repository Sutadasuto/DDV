...
function COVAREP_feature_extraction(in_dir,out_dir,sample_rate)
...
for n=1:N
    ...
    while 1
        try
            ...
            fid = fopen([out_dir filesep basename '.csv'],'w');
            commaHeader = [names;repmat({','},1,numel(names))];
            commaHeader = commaHeader(:)';
            commaHeader = commaHeader(1:length(commaHeader) - 1);
            textHeader = cell2mat(commaHeader);
            fprintf(fid,'%s\n',textHeader)
            fclose(fid)
            %write data to end of file
            dlmwrite([out_dir filesep basename '.csv'],features,'-append');
            clear features
            ...
            scale_factor=0.9;
            sample_rate = scale_factor * sample_rate;
            if (length(x)/fs) / 100.0 > sample_rate
                %write header to file
                fid = fopen([out_dir filesep basename '.csv'],'w');
                commaHeader = [names;repmat({','},1,numel(names))];
                commaHeader = commaHeader(:)';
                ...
                fprintf(fid,'%s\n',textHeader)
                fclose(fid)
                %write null data to end of file
                warning('Writing empty data to output file')
                dlmwrite([out_dir filesep basename '.csv'],zeros(1,length(names)),'-append');
                clear features
                disp([basename ' successfully analysed'])
                sample_rate = original_sample_rate;
                break
            end
            warning(['Reducing sample rate by ' num2str(scale_factor)])
            warning(['New sample rate: ' num2str(sample_rate)])
        end
    end
    close
end
