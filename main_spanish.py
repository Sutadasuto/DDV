...
if prune.confirm_files_per_modality(datasets_folder):
    ...
    print("All views processed the same files.")
else:
...
customFolds, customDicts = sa.get_cross_iterable(
    ...
    for clf in classifierList:
        ...
        if features_analysis:
            for filesList in filesLists:
                matrix = ml.complementarity_analysis(clf, datasets_folder, filesList, folds=customFolds)
        ...
        if fusion_analysis:
            ...
            matrix = ml.my_method(
                ...
                datasets_folder, customFolds, "bssd_views"
            )
            matrices.append(matrix)
            matrix = ml.my_method(
                fusion.BSSD(clf, "all_modalities", datasets_folder),
                datasets_folder, customFolds, "bssd_modalities"
            )
            matrices.append(matrix)
            matrix = ml.my_method(
                fusion.BSSD2(clf, "all_views", datasets_folder),
                datasets_folder, customFolds, "bssd_views_cv"
            )
            matrices.append(matrix)
            matrix = ml.my_method(
                fusion.BSSD2(clf, "all_modalities", datasets_folder),
                datasets_folder, customFolds, "bssd_modalities_cv"
            )
            matrices.append(matrix)
            matrix = ml.my_method(
                fusion.BSSD2_2(clf, "all_views", datasets_folder),
                datasets_folder, (customFolds, customDicts), "bssd_views_cv_subject"
            )
            matrices.append(matrix)
            matrix = ml.my_method(
                fusion.BSSD2_2(clf, "all_modalities", datasets_folder),
                datasets_folder, (customFolds, customDicts), "bssd_modalities_cv_subject"
            )
            matrices.append(matrix)
            matrix = ml.my_method(
                fusion.S4DB(clf, clf, "all_views", datasets_folder),
                datasets_folder, customFolds, "stacking_bssd_views"
            )
            matrices.append(matrix)
            matrix = ml.my_method(
                fusion.S4DB(clf, clf, "all_modalities", datasets_folder),
                datasets_folder, customFolds, "stacking_bssd_modalities"
            )
            matrices.append(matrix)
        ...
        if complementariy_analysis:
            ...
            mpaList.append(round(mpa, 1))
            cfdList.append(round(cfd, 3))
            matrix = ml.complementarity_analysis(clf, datasets_folder, files, folds=customFolds)
...
def get_boosting_objects(weak_learner, booster, customFolds, customDicts, datasets_folder, modalities):
    object_lists = []
    object_list = []
    for modality in modalities:
        object_list.append(fusion.S4DB(weak_learner, weak_learner, modality, datasets_folder, file_exceptions=["all.arff"]))
    object_list += [customFolds, "stacking_bssd"]
...
if boosting_comparison:
    ...
    matrices = Parallel(n_jobs=multiprocessing.cpu_count())(
        ...
        get_boosting_objects(weak_learner, booster, customFolds, customDicts, datasets_folder, modalities))
    ...
    #         datasets_folder, customFolds, "stacking_bssd"
    #     )
    #     matrices.append(matrix)
    #     matrix = ml.my_method(
    #         fusion.BSSD2_2(weak_learner, modality, datasets_folder, file_exceptions=["all.arff"]),
    #         datasets_folder, (customFolds, customDicts), "bssd_cv_subject"
    #     )
    #     matrices.append(matrix)
    #     matrix = ml.my_method(
    ...
    #     matrices.append(matrix)
    #     matrix = ml.complementarity_analysis(
    #         booster, datasets_folder, ["%s/all.arff" % (modality)], folds=customFolds
    #     )
    #     matrices.append(matrix)
    #     matrix = ml.concatenate_result_matrices(matrices)
